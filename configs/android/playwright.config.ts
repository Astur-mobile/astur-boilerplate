...
export default defineConfig({
  ...
  fullyParallel: false,
  workers: 1,
  outputDir: resolve(repoRoot, 'test-results/android'),
  reporter: [
    ['list'],
    ['html', { outputFolder: resolve(repoRoot, 'playwright-report/android'), open: 'never' }],
    ['junit', { outputFile: resolve(repoRoot, 'test-results/android/results.xml') }]
  ],
  use: {
    trace: 'off',
    video: 'off',
